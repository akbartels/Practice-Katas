...
// Every time a vehicle enters the parking lot, the system directs them to an available spot for their particular vehicle type, or notifies them that no spots are available.
...
// Note: There may be multiple available spots for a particular vehicle. It does not matter which spot your function chooses, as long as the spot is available. 
// And if there are no available spots, remember to return false.

const whereCanIPark = function (spots, vehicle) {
  let result = [];

  const loop = function (spotsData, spotType) {
